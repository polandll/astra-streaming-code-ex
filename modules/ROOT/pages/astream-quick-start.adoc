...
* _Optional:_ xref:astream-quick-start.adoc#deete-a-tenant[Delete an Astra Streaming tenant.]
...
To create an external Cassandra-enhanced sink for Astra Streaming topics:
...
. ... then if making a Cassandra-enhanced sink, enter the database name,
keyspace name, table name, client ID, client secret, and the mapping for the sink:
+
image::astream-name-sink2-ce.png[Add configuration details for the new CE sink]
